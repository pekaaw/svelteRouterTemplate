...
function create_each_block(ctx) {
	var li, a, t_value = ctx.page.name + "", t, a_href_value, li_class_value, dispose;
	...
	const block = {
		c: function create() {
			...
			add_location(a, file, 4, 61, 229);
			attr_dev(li, "class", li_class_value = "nav-item " + ctx.isActive(ctx.page.path));
		...
		p: function update(changed, new_ctx) {
			...
			if ((changed.pages) && li_class_value !== (li_class_value = "nav-item " + ctx.isActive(ctx.page.path))) {
	...
	dispatch_dev("SvelteRegisterBlock", { block, id: create_each_block.name, type: "each", source: "(4:12) {#each pages as page}", ctx });
	return block;
}
...
function instance($$self, $$props, $$invalidate) {
	let { pages, activePage, navigator } = $$props;
    let navigate = route => () => {
	...
	Object.keys($$props).forEach(key => {
		if (!writable_props.includes(key) && !key.startsWith('$$')) console_1.warn(`<Nav> was created with unknown prop '${key}'`);
	});
	...
	$$self.$capture_state = () => {
		return { pages, activePage, navigator, navigate, isActive };
	};
...
class Nav extends SvelteComponentDev {
	constructor(options) {
		...
		if (ctx.pages === undefined && !('pages' in props)) {
			console_1.warn("<Nav> was created without expected prop 'pages'");
		}
		if (ctx.activePage === undefined && !('activePage' in props)) {
			console_1.warn("<Nav> was created without expected prop 'activePage'");
		}
		if (ctx.navigator === undefined && !('navigator' in props)) {
			console_1.warn("<Nav> was created without expected prop 'navigator'");
		}
	}
...
function instance$1($$self, $$props, $$invalidate) {
	...
	const writable_props = ['pages', 'activePage', 'navigator'];
	Object.keys($$props).forEach(key => {
		if (!writable_props.includes(key) && !key.startsWith('$$')) console_1$1.warn(`<Layout> was created with unknown prop '${key}'`);
	});
	...
	$$self.$capture_state = () => {
		return { pages, activePage, navigator };
	};
	...
	$$self.$$.update = ($$dirty = { activePage: 1 }) => {
		if ($$dirty.activePage) { console.log(activePage); }
	};
...
class Layout extends SvelteComponentDev {
	constructor(options) {
		...
		const props = options.props || {};
		if (ctx.pages === undefined && !('pages' in props)) {
			console_1$1.warn("<Layout> was created without expected prop 'pages'");
		}
		if (ctx.activePage === undefined && !('activePage' in props)) {
			console_1$1.warn("<Layout> was created without expected prop 'activePage'");
		}
		if (ctx.navigator === undefined && !('navigator' in props)) {
			console_1$1.warn("<Layout> was created without expected prop 'navigator'");
		}
	}
...
class Blog extends SvelteComponentDev {
	constructor(options) {
		...
		dispatch_dev("SvelteRegisterComponent", { component: this, tagName: "Blog", options, id: create_fragment$4.name });
	}
}
...
const file$5 = "src\\pages\\shared\\Back.svelte";
...
function create_fragment$5(ctx) {
	...
	const block = {
		c: function create() {
			...
			add_location(button, file$5, 1, 4, 11);
...
class Back extends SvelteComponentDev {
	constructor(options) {
		...
		dispatch_dev("SvelteRegisterComponent", { component: this, tagName: "Back", options, id: create_fragment$5.name });
	}
}

/* src\pages\BlogNewEntry.svelte generated by Svelte v3.12.1 */

const file$6 = "src\\pages\\BlogNewEntry.svelte";

function create_fragment$6(ctx) {
	var t, h1, current;

	var back = new Back({ $$inline: true });
...
class BlogNewEntry extends SvelteComponentDev {
	constructor(options) {
		...
		dispatch_dev("SvelteRegisterComponent", { component: this, tagName: "BlogNewEntry", options, id: create_fragment$6.name });
	}
}
...
// (1:0) <Layout {activePage} pages={routesByLevel(0)} {navigator}>
function create_default_slot(ctx) {
	...
	const block = {
		c: function create() {
			...
			add_location(div, file$7, 1, 4, 64);
		},
	...
	dispatch_dev("SvelteRegisterBlock", { block, id: create_default_slot.name, type: "slot", source: "(1:0) <Layout {activePage} pages={routesByLevel(0)} {navigator}>", ctx });
	return block;
}
...
function create_fragment$7(ctx) {
	...
	const block = {
		...
		p: function update(changed, ctx) {
			...
			if (changed.$$scope || changed.routerTargetElement) layout_changes.$$scope = { changed, ctx };
...
function instance$2($$self, $$props, $$invalidate) {
    ...
    const activePageChange = (event) => {
        ...
        ($$invalidate('activePage', activePage = (event.state || event.uri || "home")));
    };
	...
	$$self.$$.update = ($$dirty = { activePage: 1 }) => {
		if ($$dirty.activePage) { console.log("App: activePage " + activePage); }
	};
